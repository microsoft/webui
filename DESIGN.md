...
The framework-independent router always uses `mode: 'same-origin'` for partial,
template and speculative preload fetches, including redirects. It stages the
actual condition-source script and FAST/string HTML writes on detached nodes
before publishing styles, inventory or template metadata. Native sink rejection
under enforced CSP propagates through navigation and `ensureLoaded` with
full-document navigation guidance; a rejected NDJSON registration cancels and
releases its reader. Non-enforced documents preserve nonce-based source execution
and FAST registration, even when the browser supports Trusted Types or the
framework has created its private policy. No global marker, capability probe,
replacement policy or check of the temporary function registry grants an exception.
