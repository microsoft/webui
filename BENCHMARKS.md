...
The streaming benchmark's `transport` group uses a persistent concurrent
consumer and the default four-slot channel. It includes large individual raw
and attribute writes, as well as many small writes; final consumption and
backpressure are timed, thread startup is not. `transport_tiny` isolates the
small-write path without cross-thread scheduling. For focused iteration:

```bash
cargo bench -p microsoft-webui --bench streaming_bench -- transport --save-baseline before
cargo bench -p microsoft-webui --bench streaming_bench -- transport --baseline before
```

The older `writer_paths` group and `streaming-resource` example size their
channels to hold an entire response before draining it. Use them for writer
and allocation overhead, not as evidence of slow-consumer memory bounds.
Hard chunk limits can increase channel work for large individual values, so
report latency alongside queued-byte and idle-pool capacity bounds.
