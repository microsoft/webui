...
7. **Light DOM is the default; `<template shadowrootmode="open">` opts a component into Shadow DOM.** Omit the wrapper for ordinary Light components. Use one sole top-level open wrapper when the component needs native `<slot>` projection, Shadow encapsulation, or root Shadow events. A `<slot>` in effective Light DOM fails the build.
