...
7. **Unwrapped components use Light DOM; `<template shadowrootmode="open">` opts a component into Shadow DOM.** Omit the wrapper for ordinary Light components. Use one sole top-level open wrapper when the component needs native `<slot>` projection, Shadow encapsulation, or root host events (`@custom-event` on the component root, which also catch events targeted at the host element itself). A `<slot>` in an unwrapped component fails the build.
