...
7. **Unwrapped components default to Shadow; `--dom light` makes them scoped Light DOM while authored open wrappers stay Shadow.** In a Light build, use one sole top-level `<template shadowrootmode="open">` when the component needs native `<slot>` projection, Shadow encapsulation, CSS-heavy frequent restyling, or root host events (`@custom-event` on the component root, which also catch events targeted at the host element itself). A `<slot>` fails only in an effective Light component.
