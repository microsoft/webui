 ...
 *
 * Reads the `window.__webui.chain` array emitted by the Rust handler
 * during SSR. Each entry already contains the resolved `component`,
 * `path`, `params`, etc.
 */
export function buildChainFromSSR(): RouteChainEntry[] {
  const meta = window.__webui;
  if (!meta?.chain || !Array.isArray(meta.chain)) return [];
...
function hydrateChainFromJSON(entries: RouteChainEntry[]): RouteChainEntry[] {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (el) {
      entry.el = el;
      if (entry.component) {
        entry.compEl = el.querySelector(entry.component) ?? undefined;
      }
...
export function findOrCreateRouteElement(
  ...
  if (parent.el) {
    ...
    if (compEl) {
      ...
      // Single pass: prefer exact (component+path) match, fall back to
      // weaker component-only match if no exact match exists.
      let exactMatch: HTMLElement | null = null;
      let componentMatch: HTMLElement | null = null;
      const wantPath = entry.path || null;
      for (let i = 0; i < allRoutes.length; i++) {
        const child = allRoutes[i] as HTMLElement;
        if (child.getAttribute('component') !== entry.component) continue;
        if (componentMatch === null) componentMatch = child;
        if (child.getAttribute('path') === wantPath) {
          exactMatch = child;
          break;
        }
      }
...
export function paramsEqual(
  ...
  // Avoid Object.keys() allocations by counting + comparing in one pass.
  let countA = 0;
  for (const k in a) {
    countA++;
