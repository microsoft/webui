...
The router works with both WebUI Framework (`@microsoft/webui-framework`) and FAST 3 (`@microsoft/fast-element`) components. It's a separate package because it's only needed for apps with client-side navigation.
