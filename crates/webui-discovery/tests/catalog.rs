...
fn fast_keeps_manifest_names_and_special_template_style_paths() -> TestResult {
    ...
    fs::write(
        package.join("dist/card.template.html"),
        "<f-template><template>Card</template></f-template>",
    )?;
    fs::write(
...
#[test]
fn fast_prefers_standard_exported_template_over_webui_variant() -> TestResult {
    let root = tempfile::tempdir()?;
    ...
    fs::write(
        ...
        "exports": {
            ".": "./dist/esm/button.js",
            "./template.html": {"default": "./dist/button.template.html"},
    ...
    let standard = "<f-template name=\"fast-button\"><template>Standard</template></f-template>";
    fs::write(package.join("dist/button.template.html"), standard)?;
    fs::write(
    ...
    assert_eq!(result.components[0].html_content, standard);
    assert_eq!(
        result.components[0].css_content.as_deref(),
        Some(":host { color: blue; }")
    );
    ...
    let inferred =
        discover_source_with_plugin("@fixture/button", root.path(), &FastDiscoveryPlugin::new())?;
    assert_eq!(inferred.components[0].html_content, standard);
    assert_eq!(
    ...
    fs::remove_file(package.join("dist/button.template.html"))?;
    assert!(discover_source_with_plugin(
        "@fixture/button",
        root.path(),
        &FastDiscoveryPlugin::new()
...
#[test]
fn fast_rejects_invalid_standard_template_exports_without_falling_back() -> TestResult {
    let root = tempfile::tempdir()?;
    ...
    fs::write(
        ...
        "<f-template name=\"fast-button\"><template>Sibling</template></f-template>",
    )?;
    for target in [r#""../escape.template.html""#, "true"] {
        fs::write(
            package.join("package.json"),
            format!(
                r#"{{"customElements":"custom-elements.json","exports":{{"./template.html":{target}}}}}"#
            ),
        )?;
        assert!(discover_source_with_plugin(
            "@fixture/invalid",
