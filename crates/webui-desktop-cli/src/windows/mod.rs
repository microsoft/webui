...
use state::{FrameState, STATE_APP_ID};
...
pub(crate) fn run_frame(frame: DesktopFrame) -> Result<()> {
    ...
    let saved = state::load_saved_state(&frame.window);
    let window_frame = FrameWindow::new(&frame.window, saved.as_ref())?;
    install_wakeup(&frame, window_frame.hwnd);
    ...
    let web_resource_requested =
        protocol::register_runtime_handler(&environment, &webview, frame.runtime)?;
...
fn install_wakeup(frame: &DesktopFrame, hwnd: windows::Win32::Foundation::HWND) {
