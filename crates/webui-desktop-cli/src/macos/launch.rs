...
use webui_desktop::{DesktopEvent, WindowOptions, WindowState, DRAG_REGION_SCRIPT};
...
use super::state::{apply_state, capture_state, display_bounds, state_store};
...
pub(super) fn build_window_and_webview(delegate: &DesktopAppDelegate, app: &NSApplication) {
    ...
    let restored_state = restore_state_if_enabled(&ivars.options, &ivars.title, mtm);
    ...
    app.setMainMenu(Some(&build_main_menu(mtm, &ivars.shell.menus, &webview)));
...
pub(super) fn persist_window_state_if_enabled(delegate: &DesktopAppDelegate) {
    let ivars = delegate.ivars();
    if !ivars.options.remember_state {
        return;
    }
    ...
    let _ = state_store(&ivars.title.to_string()).save(&state);
}
