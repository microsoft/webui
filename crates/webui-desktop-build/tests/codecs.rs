...
use webui_desktop::ipc::IpcCodec;
...
fn generated_codecs_preserve_extremes_and_golden_binary() {
    ...
    if std::env::var_os("WEBUI_UPDATE_IPC_FIXTURE").is_some() {
        ...
        assert_eq!(Item::decode_ipc(typescript.as_slice()).unwrap(), item);
    }
    assert_eq!(bytes, std::fs::read(path).unwrap());
    assert_eq!(Item::decode_ipc(bytes.as_slice()).unwrap(), item);
    assert_eq!(<()>::decode_ipc(&[][..]).unwrap(), ());
...
fn protobuf_nonfinite_and_all_numeric_encodings() {
    ...
    let decoded = Metrics::decode_ipc(metrics.encode_ipc().as_slice()).unwrap();
