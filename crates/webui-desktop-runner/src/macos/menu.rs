...
pub(super) fn build_main_menu(
    ...
    let target = DesktopMenuTarget::new(mtm, webview.clone());
    for menu in menus {
...
fn build_menu(
    ...
    for item in &menu.items {
        ...
        if let Some(command) = &item.command {
            ...
            // SAFETY: `target` is retained for the app lifetime by the caller,
...
define_class!(
    ...
    impl DesktopMenuTarget {
        ...
        fn perform_desktop_menu_command(&self, sender: &NSMenuItem) {
            ...
            // SAFETY: WebKit evaluates this owned, well-formed script on its
            // main-thread web view.
            unsafe {
                ivars
                    .webview
...
impl DesktopMenuTarget {
    fn new(mtm: MainThreadMarker, webview: Retained<WKWebView>) -> Retained<Self> {
...
mod tests {
    ...
    fn command_script_embeds_id_as_json() {
        let script = menu_command_script("open-preferences");
