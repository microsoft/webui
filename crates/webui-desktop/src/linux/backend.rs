...
pub(crate) fn run_frame(frame: DesktopFrame) -> Result<()> {
    ...
    let application = match frame
        .app_id
        .as_deref()
        .filter(|id| gio::Application::id_is_valid(id))
    {
        Some(id) => Application::builder().application_id(id),
        // An absent/non-D-Bus identity must not make unrelated apps share a
        // fixed GApplication id. Persistence still uses the explicit frame id.
...
fn connect_theme_events(webview: &WebView, events: &crate::EventRegistry) {
...
fn is_allowed_navigation_url(url: &str) -> bool {
    crate::is_allowed_navigation_url(url, super::APP_ORIGIN)
}
