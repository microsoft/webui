...
use webui_protocol::{ConditionExpr, Predicate, ComparisonOperator, LogicalOperator};
...
impl ConditionParser {
    ...
    pub fn parse(&self, input: &str) -> Result<ConditionExpr> {
        ...
        Err(ParserError::Parse(format!("Failed to parse condition: '{}'", input)))
    }
    ...
    fn parse_identifier(&self, input: &str) -> Result<ConditionExpr> {
        ...
        Err(ParserError::Parse(format!("Invalid identifier value: '{}'", input)))
    }
    ...
    fn split_by_logical_op(&self, input: &str, operators: &[&str], op: LogicalOperator) -> Option<ConditionExpr> {
                            ...
                            if i + op_str.len() <= chars.len() {
                                ...
                                if slice == *op_str {
                                    ...
                                    let is_standalone = (i == 0 || !chars[i-1].is_alphanumeric()) && 
                                        (i + op_str.len() == chars.len() || !chars[i + op_str.len()].is_alphanumeric());
...
mod tests {
    ...
    fn test_parse_identifier() {
        ...
        let result = parser.parse("isLoggedIn").unwrap();
        
        assert!(matches!(result, ConditionExpr::Identifier { value } if value == "isLoggedIn"));
        
        // Test dot notation
        let result = parser.parse("user.isAdmin").unwrap();
    ...
    fn test_parse_predicate() {
        ...
        for (input, expected_op) in test_cases {
            ...
            assert!(matches!(result, ConditionExpr::Predicate(pred) if pred.operator == expected_op));
        }
        
        // Test string values with quotes
        let result = parser.parse("name == \"John\"").unwrap();
    ...
    fn test_parse_compound() {
        ...
        // Test AND condition
        let result = parser.parse("isLoggedIn && age > 18").unwrap();
        assert!(matches!(result, ConditionExpr::Compound { left, op, right } if 
        ...
        let result = parser.parse("status == 'premium' or isAdmin").unwrap();
        assert!(matches!(result, ConditionExpr::Compound { left, op, right } if 
    ...
    fn test_complex_expressions() {
        ...
        let result = parser.parse("age > 18 && isVerified || isAdmin").unwrap();
        assert!(matches!(result, ConditionExpr::Compound { .. }));
        
        // Negated expression
        let result = parser.parse("!isVerified").unwrap();
        assert!(matches!(result, ConditionExpr::Not(_)));
    }
