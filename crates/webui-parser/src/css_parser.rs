...
impl CssParser {
    ...
    pub fn parse(&mut self, css_content: &str) -> Result<WebUIStreamRecords> {
        ...
        let _tree = self.parser.parse(css_content, None)
    ...
    pub fn process_css(&mut self, css_content: &str, streams: &mut WebUIStreamRecords) -> Result<()> {
        // Parse CSS with tree-sitter
        let tree = self.parser.parse(css_content, None)
            .ok_or_else(|| ParserError::Css("Failed to parse CSS".to_string()))?;
    ...
    fn process_css_rules(&self, _tree: Tree, _source: &str, _streams: &mut WebUIStreamRecords) -> Result<()> {
    ...
    pub fn parse_inline_css(&mut self, style_content: &str) -> Result<String> {
        ...
        let _tree = self.parser.parse(style_content, None)
