...
impl ComponentRegistry {
    ...
    pub fn register_component_from_paths<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        // A tag that is already registered wins; re-registering it (e.g. the
        // same component rediscovered by an overlapping scan path) is a no-op.
...
mod tests {
    ...
    fn test_duplicate_component_from_strings() {
        ...
        // Registering a second component with the same tag is a no-op — the
        // first registration wins. Duplicate detection is keyed on the tag
        // name, not on content.
        ...
        // The original component is retained unchanged.
    ...
    fn test_duplicate_component_from_paths() {
        ...
        // The same tag name from a different path is a no-op — the first
        // registration wins. This is what lets overlapping scan paths (the app
        // directory and an ancestor `--components` path) rediscover the same
